...
import pokedox from './pokemon-species.json' assert {type: 'json'};
...
pokedex.forEach(poke => {
  availableTags.push(poke.name.english)
});
...
let pokemons = JSON.parse(localStorage.getItem("pokemons")) ?? structuredClone(pokedex)// con el clon evito tener referencias que contradigan los datos a la hora de reiniciar el juego
...
function loadQuestion() {
  number = Math.floor(Math.random() * pokemons.length);
   ...
   //   console.log("pregunta repetida, cambiando " + pokemonSelected.name.english)
...
export function checkAnswer() {
  ...
  if (pokemonSelected.name.english.toLowerCase() == answerInput.value.toLowerCase()) {
...
function nextQuestion() {
  ...
  answerText.textContent = pokemonSelected.name.english.toLowerCase()
    == answerInput.value.toLowerCase() ?
    `Correcto,  es   ${pokemonSelected.name.english}` :
    `Has perdido,  es  ${pokemonSelected.name.english} `
...
function resetGame() {
  ...
  pokemons = structuredClone(pokedex)// reseteo la lista para que todas las preguntas esten sin responder
  puntos = 0;
  pokemon_index = 0
 pokemonSelected = null;
  saveData()
...
async function ChargeImage() {
let response = await fetch(`https://pokeapi.co/api/v2/pokemon/${pokedox.results[pokemonSelected.id - 1].name}`)
let data  = await response.json()
let image = await data.sprites.other["official-artwork"].front_default
console.log(pokemonSelected.name.english)
